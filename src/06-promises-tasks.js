...
function chainPromises(array, action) {
  function chainPromiseHelper(promises, acc) {
    ...
    return promise
      .then((value) => {
        const nextAcc = acc === undefined ? value : action(acc, value);
        return chainPromiseHelper(promises.slice(1), nextAcc);
      })
      .catch(() => chainPromiseHelper(promises.slice(1), acc));
  }
  return chainPromiseHelper(array, undefined);
}
